...
class AETrainer(object):
	def __init__(self, model, inputs, mask=None, batch_size=64):
		...
		self.costfunc = model.cost
		self.updates = model.updates

	def get_training_function(self, cost, updates):
		given = {model.inputs : self.inputs[self.i:self.i+self.bs]}

		if model.mask:
			given[model.mask] = self.mask[self.i:self.i+self.bs]

		return theano.function([self.i, self.bs], cost, updates, givens=given)

	def run_epochs(self, min_epochs=50, min_improvement=1.001, 
					lr_decay=0.0, decay_modulo=25):
		start = time.time()
		training_function = self.get_training_function(self.costfunc, self.updates)
		...
		while self.steps < epoch_stop:
			...
			if (np.mean(costs) * min_improvement) < self.costs[-1][1]:
				...
				self.model.learning_rate *= lr_decay
