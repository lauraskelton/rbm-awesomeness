...
def makeGoogleBeerMap(loc,itemPrefs,style,abv=None,ibu=None):
    output=open('data/beergmap.html','w')
    output.write('<!DOCTYPE html>\n')
    output.write('<html>\n')
    output.write('\t<head>\n')
    output.write('\t\t<title>Beer Map</title>\n')
    output.write('\t\t<style>\n')
    output.write('\t\t\thtml, body, #map-canvas {\n')
    output.write('\t\t\t\theight: 100%;\n')
    output.write('\t\t\t\tmargin: 0px;\n')
    output.write('\t\t\t\tpadding: 0px\n')
    output.write('\t\t\t}\n')
    output.write('\t\t</style>\n')
    output.write('\t\t<script src="https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false"></script>\n')
    output.write('\t\t<script type="text/javascript" src="../beermap/infobox.js"></script>\n')
    output.write('\t\t<script>\n')
    output.write('\n')
    output.write('function CoordMapType() {\n')
    output.write('}\n')
    output.write('\n')
    output.write('CoordMapType.prototype.tileSize = new google.maps.Size(256,256);\n')
    output.write('CoordMapType.prototype.maxZoom = 11;\n')
    output.write('CoordMapType.prototype.minZoom = 5;\n')
    output.write('\n')
    output.write('CoordMapType.prototype.getTile = function(coord, zoom, ownerDocument) {\n')
    output.write('\tvar div = ownerDocument.createElement(\'div\');\n')
    output.write('\tdiv.innerHTML = \'\';\n')
    output.write('\tdiv.style.width = this.tileSize.width + \'px\';\n')
    output.write('\tdiv.style.height = this.tileSize.height + \'px\';\n')
    output.write('\tdiv.style.fontSize = \'10\';\n')
    output.write('\tdiv.style.borderStyle = \'none\';\n')
    output.write('\tdiv.style.backgroundColor = \'#E5E3DF\';\n')
    output.write('\treturn div;\n')
    output.write('};\n')
    output.write('\n')
    output.write('CoordMapType.prototype.name = \'Beer\';\n')
    output.write('CoordMapType.prototype.alt = \'Beer Map Type\';\n')
    output.write('\n')
    output.write('var map;\n')
    output.write('var coordinateMapType = new CoordMapType();\n')
    output.write('\n')
    output.write('var beerMapData = [\n')
    for beer in itemPrefs:
        output.write('\t{latitude: '+str(float((loc[beer][0])*15))+', ')
        output.write('longitude: '+str(float((loc[beer][1])*15))+', ')
        output.write('title: "'+str(beer)+'", ')
        output.write('description: "'+str(style[beer].replace("&","and"))+'",')
        ...
        output.write('color: "rgb('+str(r)+','+str(g)+','+str(b)+')"},')
        output.write('\n')
    
    output.write('];\n')
    output.write('\n')
    output.write('var mapCenter = new google.maps.LatLng(11.9807276122, 4.99151301465);\n')
    output.write('\n')
    output.write('function initialize() {\n')
    output.write('\tvar mapOptions = {\n')
    output.write('\t\tzoom: 5,\n')
    output.write('\t\tcenter: mapCenter,\n')
    output.write('\t\tstreetViewControl: false,\n')
    output.write('\t\tmapTypeId: \'coordinate\',\n')
    output.write('\t\tmapTypeControlOptions: {\n')
    output.write('\t\t\tmapTypeIds: [\'coordinate\'],\n')
    output.write('\t\t\tstyle: google.maps.MapTypeControlStyle.DROPDOWN_MENU\n')
    output.write('\t\t}\n')
    output.write('\t};\n')
    output.write('\tmap = new google.maps.Map(document.getElementById(\'map-canvas\'),mapOptions);\n')
    output.write('\n')
    output.write('\tfor (var i=0;i<beerMapData.length;i++)\n')
    output.write('\t{\n')
    output.write('\t\tvar myOptions = {\n')
    output.write('\t\t\tcontent: beerMapData[i][\'title\']+"<br/>("+beerMapData[i][\'description\']+")"\n')
    output.write('\t\t\t,boxStyle: {\n')
    output.write('\t\t\t\tborder: "1px solid black"\n')
    output.write('\t\t\t\t,textAlign: "center"\n')
    output.write('\t\t\t\t,fontSize: "6pt"\n')
    output.write('\t\t\t\t,width: "60px"\n')
    output.write('\t\t\t\t,backgroundColor: beerMapData[i][\'color\']\n')
    output.write('\t\t\t}\n')
    output.write('\t\t\t,disableAutoPan: true\n')
    output.write('\t\t\t,pixelOffset: new google.maps.Size(-25, 0)\n')
    output.write('\t\t\t,position: new google.maps.LatLng(beerMapData[i][\'latitude\'], beerMapData[i][\'longitude\'])\n')
    output.write('\t\t\t,closeBoxURL: ""\n')
    output.write('\t\t\t,isHidden: false\n')
    output.write('\t\t\t,pane: "mapPane"\n')
    output.write('\t\t\t,enableEventPropagation: true\n')
    output.write('\t\t};\n')
    output.write('\t\tvar ibLabel = new InfoBox(myOptions);\n')
    output.write('\t\tibLabel.open(map);\n')
    output.write('\t}\n')
    output.write('\n')
    output.write('map.mapTypes.set(\'coordinate\', coordinateMapType);\n')
    output.write('}\n')
    output.write('\n')
    ...
    output.write('\t</head>\n')
    output.write('\t<body>\n')
    output.write('\t\t<div id="map-canvas"></div>\n')
    output.write('\t</body>\n')
    output.write('</html>\n')
