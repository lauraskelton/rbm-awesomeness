...
def writeBeerMapData(beerWeights):
    ...
    output = []
    hOffset = 0
    for beer, ratingWeights in beerWeights.iteritems():
    	vOffset = 0
    	for ratingWeight in ratingWeights:
    		loc = [hOffset, vOffset]
    		backgroundColor = getBackgroundColor(ratingWeight, beerWeights.max(), beerWeights.min())
    		output.append(outputBeer(loc,beer,style[beer],'#000000',backgroundColor))
    		vOffset += 1
    	hOffset += 1
    return ''.join(output)
